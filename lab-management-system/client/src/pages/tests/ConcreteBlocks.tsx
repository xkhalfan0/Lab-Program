        ...
        <div className="flex items-start justify-between flex-wrap gap-3">
          <div>
            ...
            <p className="text-slate-500 text-sm mt-1">
              {ar ? "BS EN 6073 | أمر التوزيع:" : "BS EN 6073 | Distribution:"} {dist?.distributionCode ?? `DIST-${distId}`}
            </p>
          </div>
              ...
              <tbody>
                {computedRows.map((row, idx) => (
                  <tr key={row.id} className={idx % 2 === 0 ? "bg-white" : "bg-slate-50/50"}>
                    ...
                    <td className="border border-slate-200 px-1 py-1 text-center">
                      <Button variant="ghost" size="sm" className="h-8 w-8 p-0 text-red-600 hover:text-red-700" onClick={() => removeRow(row.id)} disabled={rows.length <= 1}>
                        <Trash2 size={12} />
                      </Button>
                    </td>
                  </tr>
