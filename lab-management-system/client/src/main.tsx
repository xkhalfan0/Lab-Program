...
const trpcClient = trpc.createClient({
  links: [
    httpBatchLink({
      ...
      headers() {
        // Only attach sector token when on sector portal pages
        const isSectorPage = typeof window !== "undefined" && window.location.pathname.startsWith("/sector");
        if (isSectorPage) {
          const sectorToken = localStorage.getItem("sector_token");
          if (sectorToken) {
            return { Authorization: `Bearer ${sectorToken}` };
          }
        }
        return {};
      },
