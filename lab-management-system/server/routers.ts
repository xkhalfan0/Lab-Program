...
/** Reception: new lab order (one sample + lab_order + items). Used by `orders.create` and `orders.createBatch`. */
const labOrderReceptionCreateInputSchema = z.object({
...
type LabOrderReceptionCreateInput = z.infer<typeof labOrderReceptionCreateInputSchema>;

async function runLabOrderReceptionCreate(
  ctx: { user: { id: number; role: string; name: string | null }; req: { ip?: string } },
  input: LabOrderReceptionCreateInput
) {
